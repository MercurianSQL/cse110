...
shopping_cart_item_price = 0.00 # Variable to retrieve the item price in shopping_cart_item_prices
input_item_price
sci_action = 0
sci_py_index = 0
...
while five_menu_action != 5:
    if five_menu_action == 1: #1. Add item
        ...
        for shopping_cart_item_price in shopping_cart_item_prices:
            cart_subtotal += float(shopping_cart_item_price)
            print(f"Your cart subtotal is ${cart_subtotal:0.2f}.")
            five_menu_action = 0
        ...
        shopping_cart_item_price = shopping_cart_item_prices[sci_py_index]
        print()
        shopping_cart_items.pop(sci_py_index)
        shopping_cart_item_prices.pop(sci_py_index)
        print(f"Item {sci_action}. {shopping_cart_item}, ${shopping_cart_item_price:0.02f} has been removerd from your cart.")
        five_menu_action = 0

    elif five_menu_action == 4: #4. Compute total  # cost cost is messed up becasue of NYOP: correct!
        print("Your Shopping Cart:")
        if len(shopping_cart_items) == 0:
            print(f"Your shopping cart is empty. Total is ${shopping_cart_item_price:0.2f}")
            five_menu_action = 0
        else: 
            for shopping_cart_item_price in shopping_cart_item_prices:
                cart_subtotal += float(shopping_cart_item_price)
            ...
            if purchase == "1":
                ...
                print(f"Tax on your purchase of ${cart_subtotal:0.2f} is ${cart_taxed:0.2f}")
